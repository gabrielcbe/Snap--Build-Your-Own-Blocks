...
var LAMPADA_RGB = 'lampadargb';
var LAMPADA_BRANCA = 'lampadabranca';
...
function registraConexaoLampBLE(dado) {
    ...
    if (msg[1]) {
        ...
        if (
            parseInt(msg[2].substring(8)) < 10 &&
      msg[2].substring(8).indexOf('0') != 0
        )
            estacaoLampBLE = '0' + msg[2].substring(8);
        else estacaoLampBLE = msg[2].substring(8);
    }
...
function statusConnectionLampBLE(urlConexaoLampBLEArg) {
    if (
        !urlConexaoLampBLEArg ||
    urlConexaoLampBLEArg == '' ||
    urlConexaoLampBLEArg == 'localhost'
    ) {
...
function waitForSocketConnectionLampBLE(socket, callback) {
